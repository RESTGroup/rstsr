...
use crate::threading::with_num_threads as mkl_with_num_threads;
use core::any::TypeId;
    ...
    TC: Mul<TC, Output = TC> + Add<TC, Output = TC> + Zero + PartialEq,
{
    ...
    match (la.ndim(), lb.ndim(), lc.ndim()) {
        (1, 1, 0) => {
            ...
            return mkl_with_num_threads(nthreads, || {
    ...
    if n_task >= 4 * nthreads {
        ...
        mkl_with_num_threads(nthreads, || -> Result<()> {
