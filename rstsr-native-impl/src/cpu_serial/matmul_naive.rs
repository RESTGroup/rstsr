    ...
    TC: Mul<TC, Output = TC> + Add<TC, Output = TC>,
{
    ...
    for i_m in 0..m {
        for i_n in 0..n {
            ...
            c[idx_c] = if beta.is_zero() { TC::zero() } else { beta.clone() * c[idx_c].clone() };
        }
        for i_k in 0..k {
    ...
    TC: Mul<TC, Output = TC> + Add<TC, Output = TC>,
{
    ...
    // BLAS convention: when beta is zero, C is not read — it may hold
    // uninitialized or otherwise undefined values.
    let mut sum = if beta.is_zero() { TC::zero() } else { beta * c[idx_c].clone() };
    for i in 0..n {
...
/* #region write-only (uninit) naive matmul kernels */

// These kernels write `c = alpha * (a @ b)` into `MaybeUninit` storage and
// never read `c`. They back the fresh-output matmul path
// (`DeviceMatMulAPI::matmul_uninit`), where the output buffer is
// uninitialized by construction.

#[allow(clippy::too_many_arguments)]
pub fn matmul_naive_uninit_cpu_serial<TA, TB, TC, DA, DB, DC>(
    ...
    TC: Mul<TC, Output = TC> + Add<TC, Output = TC>,
{
    ...
    match (la.ndim(), lb.ndim(), lc.ndim()) {
        (1, 1, 0) => {
            ...
            let lb = &lb.clone().into_dim::<Ix1>().unwrap();
            let lc = &lc.clone().into_dim::<Ix0>().unwrap();
            inner_dot_naive_uninit_cpu_serial(c, lc, a, la, b, lb, alpha)?;
        },
        (2, 2, 2) => {
            // rule 2: matrix multiplication
            let la = &la.clone().into_dim::<Ix2>().unwrap();
            let lb = &lb.clone().into_dim::<Ix2>().unwrap();
            let lc = &lc.clone().into_dim::<Ix2>().unwrap();
            gemm_naive_uninit_cpu_serial(c, lc, a, la, b, lb, alpha)?;
        },
        _ => {
            // broadcasted rules 3..7: the config resolves the rule, broadcasts
            // the batch (`rest`) dims against `lc`, and hands us 2-D matmul
            ...
            for (ia_rest, ib_rest, ic_rest) in izip!(ita_rest, itb_rest, itc_rest) {
                ...
                gemm_naive_uninit_cpu_serial(c, &lc_m, a, &la_m, b, &lb_m, alpha.clone())?;
            }
        },
    }
    Ok(())
...
#[allow(clippy::too_many_arguments)]
pub fn gemm_naive_uninit_cpu_serial<TA, TB, TC>(
    c: &mut [MaybeUninit<TC>],
    lc: &Layout<Ix2>,
    a: &[TA],
    la: &Layout<Ix2>,
...
#[allow(clippy::too_many_arguments)]
pub fn gemv_naive_uninit_cpu_serial<TA, TB, TC>(
    c: &mut [MaybeUninit<TC>],
    lc: &Layout<Ix1>,
    a: &[TA],
    la: &Layout<Ix2>,
...
#[allow(clippy::too_many_arguments)]
pub fn gevm_naive_uninit_cpu_serial<TA, TB, TC>(
    c: &mut [MaybeUninit<TC>],
    lc: &Layout<Ix1>,
    a: &[TA],
    la: &Layout<Ix1>,
    ...
    TC: Mul<TC, Output = TC> + Add<TC, Output = TC>,
{
    ...
    let sb = lb.shape();
    let sa = la.shape();
    rstsr_assert_eq!(sb[0], sa[0], InvalidLayout)?;
    let (n, k) = (sb[1], sb[0]);
...
#[allow(clippy::too_many_arguments)]
pub fn inner_dot_naive_uninit_cpu_serial<TA, TB, TC>(
    c: &mut [MaybeUninit<TC>],
    lc: &Layout<Ix0>,
    a: &[TA],
    la: &Layout<Ix1>,
