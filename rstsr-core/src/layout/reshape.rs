...
fn pop_layout_in(shape_in: &mut Vec<usize>, stride_in: &mut Vec<isize>) -> (usize, isize) {
    rstsr_assert_eq!(shape_in.len(), stride_in.len(), InvalidLayout);
    rstsr_assert!(!shape_in.is_empty(), InvalidLayout);
...
fn pop_shape_out(
    ...
    rstsr_assert!(!shape_out.is_empty(), InvalidLayout);
...
fn complicated_reshape(shape_out: &[usize], layout_in: &Layout<IxD>) -> Option<Layout<IxD>> {
    ...
    rstsr_assert!(shape_out.is_empty(), RuntimeError);
    rstsr_assert_eq!(stride_out.len(), shape_out_ref.len(), RuntimeError);
