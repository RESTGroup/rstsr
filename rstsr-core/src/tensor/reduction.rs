...
//! local `trait_reduction!` macro; each family exposes all-element (`_all`),
//! axis-wise (`_axes`), and fallible (`_f`) variants (`unraveled_argmin` /
//! `unraveled_argmax` come from the `trait_reduction_arg!` macro).
//! [`allclose`] is hand-written and has no `_axes` variants.
